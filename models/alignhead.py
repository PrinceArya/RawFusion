...
class MultiCrossAlign_head_atttrans_res1sepalign(nn.Module):
    def __init__(self, in_c, num_heads=4, dim_align=64):
        ...
        self.conv_f4 = nn.Conv2d(in_c, dim_align, 3, 1, 1)
        self.conv_f5 = nn.Conv2d(in_c, dim_align, 3, 1, 1)
        self.conv_f6 = nn.Conv2d(in_c, dim_align, 3, 1, 1)
        self.conv_f7 = nn.Conv2d(in_c, dim_align, 3, 1, 1)
        self.conv_f8 = nn.Conv2d(in_c, dim_align, 3, 1, 1)
        self.conv_f9 = nn.Conv2d(in_c, dim_align, 3, 1, 1)
        ...
        self.pyramid4 = Pyramid(in_channels=dim_align, n_feats=dim_align)
        self.pyramid5 = Pyramid(in_channels=dim_align, n_feats=dim_align)
        self.pyramid6 = Pyramid(in_channels=dim_align, n_feats=dim_align)
        self.pyramid7 = Pyramid(in_channels=dim_align, n_feats=dim_align)
        self.pyramid8 = Pyramid(in_channels=dim_align, n_feats=dim_align)
        self.pyramid9 = Pyramid(in_channels=dim_align, n_feats=dim_align)
        ...
        self.align3 = Pyramid_CrossattAlign_Atttrans(scales=3, num_feats=dim_align, num_heads=4, window_size=8)
        self.align4 = Pyramid_CrossattAlign_Atttrans(scales=3, num_feats=dim_align, num_heads=4, window_size=8)
        self.align5 = Pyramid_CrossattAlign_Atttrans(scales=3, num_feats=dim_align, num_heads=4, window_size=8)
        self.align6 = Pyramid_CrossattAlign_Atttrans(scales=3, num_feats=dim_align, num_heads=4, window_size=8)
        self.align7 = Pyramid_CrossattAlign_Atttrans(scales=3, num_feats=dim_align, num_heads=4, window_size=8)
        self.align8 = Pyramid_CrossattAlign_Atttrans(scales=3, num_feats=dim_align, num_heads=4, window_size=8)
    ...
    def forward(self, x1, x2, x3, x4, x5, x6, x7, x8, x9):
        ...
        x4_feature_list = self.pyramid4(x4)
        x5_feature_list = self.pyramid5(x5)
        x6_feature_list = self.pyramid6(x6)
        x7_feature_list = self.pyramid7(x7)
        x8_feature_list = self.pyramid8(x8)
        x9_feature_list = self.pyramid9(x9)
        ...
        aligned_x4 = self.align3(x2_feature_list, x4_feature_list, patch_ratio_list=[2,2,2])
        aligned_x5 = self.align4(x2_feature_list, x5_feature_list, patch_ratio_list=[2,2,2])
        aligned_x6 = self.align5(x2_feature_list, x6_feature_list, patch_ratio_list=[2,2,2])
        aligned_x7 = self.align6(x2_feature_list, x7_feature_list, patch_ratio_list=[2,2,2])
        aligned_x8 = self.align7(x2_feature_list, x8_feature_list, patch_ratio_list=[2,2,2])
        aligned_x9 = self.align8(x2_feature_list, x9_feature_list, patch_ratio_list=[2,2,2]) 
        
        return [aligned_x1, x2, aligned_x3, aligned_x4, aligned_x5, aligned_x6, aligned_x7, aligned_x8, aligned_x9]
...
class SpatialAttentionModule(nn.Module):
    ...
    def __init__(self, dim):
        super().__init__()
        self.att1 = nn.Conv2d(dim * 2, dim, kernel_size=3, padding=1, bias=True)
        self.att2 = nn.Conv2d(dim, dim, kernel_size=3, padding=1, bias=True)
...
class GatingModule(nn.Module):
    ...
    def __init__(self, dim):
        super().__init__()
        self.gating = nn.Conv2d(dim * 2, dim, kernel_size=1, bias=True)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x1, x2):
        f_cat = torch.cat((x1, x2), 1)
        gate = self.sigmoid(self.gating(f_cat))
        out = gate*x1 + (1-gate)*x2
        return out            
    

class MultiScaleGatedModule(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.gate1 = GatingModule(dim)
        self.ds1_1 = nn.Conv2d(dim, dim, kernel_size = 3, stride=2, padding=1)
        self.ds1_2 = nn.Conv2d(dim, dim, kernel_size = 3, stride=2, padding=1)
        self.gate2 = GatingModule(dim)
        self.ds2_1 = nn.Conv2d(dim, dim, kernel_size = 3, stride=2, padding=1)
        self.ds2_2 = nn.Conv2d(dim, dim, kernel_size = 3, stride=2, padding=1)
        self.gate3 = GatingModule(dim)

        self.upscale = nn.Upsample(scale_factor=2, mode='bilinear')

    def forward(self, x, xref):
        lvl1 = self.gate1(x, xref)  

        ds_x1_1 = self.ds1_1(x)
        ds_xref1_2 = self.ds1_2(xref)

        lvl2 = self.gate2(ds_x1_1, ds_xref1_2)

        ds_x2_1 = self.ds2_1(ds_x1_1)
        ds_xref2_2 = self.ds2_2(ds_xref1_2)

        lvl3 = self.gate3(ds_x2_1, ds_xref2_2)

        up_1 = self.upscale(lvl3) + lvl2
        up_2 = self.upscale(up_1) + lvl1
